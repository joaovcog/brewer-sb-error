...
public class PageWrapper<T> {
	...
	private int paginasLimite = 9;
	
	public PageWrapper(Page<T> page, HttpServletRequest httpServletRequest) {
		this.page = page;
		
		if (getTotal() < paginasLimite) {
			paginasLimite = getTotal() - 1;
		}
	...
	public int getPaginasLimite() {
		return paginasLimite;
	}
	...
	public boolean isAbaixoLimite() {
		return getTotal() <= getPaginasLimite();
	}
	
	public int getInicio() {
		return Math.max(1, (getAtual() - getPaginasLimite()) + 1);
	}

	public int getFim() {
		return getInicio() + getPaginasLimite();
	}
